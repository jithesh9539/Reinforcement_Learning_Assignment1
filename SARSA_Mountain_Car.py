...
class SARSA_Learner(object):
    ...
    def learn(self, obs, action, reward, next_obs,action1):
        discretized_obs = self.discretize(obs)
        discretized_next_obs = self.discretize(next_obs)
        td_target = reward + self.gamma * self.Q[discretized_next_obs][action1]
...
def train(agent, env):
    ...
    for episode in range(MAX_NUM_EPISODES):
        ...
        while not done:
            ...
            action1=agent.get_action(next_obs)
            agent.learn(obs, action, reward, next_obs,action1)
            obs = next_obs
            total_reward += reward
...
def test(agent, env, policy):
    ...
    while not done:
        ...
        next_obs, reward, done, info = env.step(action)
        action1=policy[agent.discretize(next_obs)]
        obs = next_obs
        total_reward += reward
    return total_reward
